...
type BtnAddPhotoProps = {
  ...
  /** ✨ 수정 모드 등에서 기본으로 선택되어 있어야 하는 URI 배열 */
...
export default function BtnAddPhoto({
  ...
  // ✨ 외부에서 initialSelected가 바뀌면 반영 (수정 화면에서 탭 back→재진입 등)
  useEffect(() => {
    if (!initialSelected) return;
    setSelectedPhotos(initialSelected.slice(0, max));
  }, [initialSelected?.join('|'), max]);
  ...
  const toggleSelect = (uri: string) => {
    setSelectedPhotos((curr) => {
      if (curr.includes(uri)) {
        return curr.filter((u) => u !== uri);
      }
      if (curr.length >= max) {
          ...
          {selectedPhotos.map((uri) => (
            <View key={uri} style={styles.selectedPhotoWrapper}>
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  setSelectedPhotos((curr) => curr.filter((u) => u !== uri))
                }
              >
                <Image
